...
npm install
npx ts-node src/migrate-payments.ts  # 1) Payments migration
set -a && source .env && set +a
PGPASSWORD="$DB_PASSWORD" psql -v ON_ERROR_STOP=1 -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USER" -d "$DB_NAME" -f db/migrate_painting_parties_child_age_to_varchar.sql  # 2) child_age type alignment migration
npm run build
...
### 3. Database
- **Run migrations in release order** before first use:
   1) `npx ts-node src/migrate-payments.ts`
   2) `PGPASSWORD="$DB_PASSWORD" psql -v ON_ERROR_STOP=1 -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USER" -d "$DB_NAME" -f db/migrate_painting_parties_child_age_to_varchar.sql`
...
- [ ] Ran database migrations (payments + child_age type alignment)
